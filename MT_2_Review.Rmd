...
If only some column names are values of a variable - then specify a range of columns that you want to pivot or columns that you want to stay fixed or identify columns by a prefix, remove the prefix and all NA's
```{r}
billboard2 <- 
  billboard %>% 
...
gapminder %>% 
  ...
  ggplot(aes(x=country, y=lifeExp, fill=continent))+ 
  geom_boxplot(alpha=0.4) + 

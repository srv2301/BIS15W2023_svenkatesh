...
ui <- fluidPage(titlePanel("Log 10 HomeRange by Taxon"),
                selectInput("x", "Select Taxon", choices=c("mammals", "birds", "lake fishes", "lizards", "marine fishes", "river fishes", "snakes", "tortoises", "turtles"), 
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    ...
    ggplot(aes(x=log10.hra)) +
      geom_density(color = "black", fill = "steelblue", alpha = 0.7) +
      theme_light(base_size=18)
  })
}
...
server <- function(input, output, session) {
  session$onSessionEnded(stopApp) #defined the function to stop running when the app is closed
  output$plot <- renderPlot({

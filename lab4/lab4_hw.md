...
colMeans(herbivore_mean, na.rm = T)
```
...
colMeans(carnivore_mean, na.rm = T)
```
...
```r
new_deer <- select(homerange, "species", "common.name")
new_deer
```
...
##    species     common.name            
##    <chr>       <chr>                  
##  1 rostrata    american eel           
##  2 poecilura   blacktail redhorse     
##  3 anomalum    central stoneroller    
##  4 funduloides rosyside dace          
##  5 cataractae  longnose dace          
##  6 masquinongy muskellunge            
##  7 pollachius  pollack                
##  8 virens      saithe                 
##  9 lineatus    lined surgeonfish      
## 10 lituratus   orangespine unicornfish
## # … with 559 more rows
```

```r
filter(new_deer, species=="alces")
```
...
## #   ²​primarymethod, ³​mean.mass.g, ⁴​log10.mass
```
The Western Worm Snake has the smallest homerange. It is generally known to be small, non-venomous and is found extensively over the southern part of the US. 

...
```
Norway has the highest average life expectancy.
...
life_expectancy_long %>% 
  ...
  mutate(life_exp_growth = `2020`-`1920`) %>% 
...
```{r}
kuwait_life_exp <- life_expectancy_long %>% 
  filter(between(year, 1920, 2020)) %>% 
  filter(country == "Kuwait") %>% 
  select(year, life_exp) %>% 
  ggplot(aes(x=year, y=life_exp, group = 1))+
  geom_line(color = "blue")+
  labs(title = "Growth in Life Expectancy in Kuwiat from 1920-2020",
       x= "Year",
       y = "Life Expectancy")
```
...
population%>% 
  ...
  theme(axis.title = element_text(size=8),
        axis.text.x = element_text(size=3, angle = 80, hjust=1))+
  labs(title = "Highest population growth of the top 5 countries from 1920-2020",
       x="year",
       y = "Population Growth")
```
...
income_long %>% 
  ...
  pivot_wider(names_from = year,
              values_from = income) %>% 
  select(country, `1920`, `2020`) %>% 
  mutate(income_growth = `2020`-`1920`) %>% 
...
income %>% 
  ...
  theme(axis.title = element_text(size=8),
        axis.text.x = element_text(size=3, angle = 80, hjust=1))+
  labs(title = "Highest income growth for the top 5 countries from 1920-2020",
       x="year",
       y = "Population Growth")
```
...
Comparison of the range of income by country
```{r}

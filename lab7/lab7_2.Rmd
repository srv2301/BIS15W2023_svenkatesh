...
amniota <- readr::read_csv("data/amniota.csv")#%>% 
  #na_if("-999") #this would help replace the NA as soon as its imported
```
...
```{r}
#NA's are listed as -999
hist(amniota$weaning_weight_g)
```
...
```{r}
#It does not seem likley. NA's are represented as -999 in the data.
glimpse(life_history)
```

3. Replace the placeholder values with NA and recheck.
```{r}
life_history_new <- life_history %>% 
  na_if("-999")

naniar::miss_var_summary(life_history_new)
```

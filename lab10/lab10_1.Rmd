...
life_history %>% 
  group_by(order) %>% 
  summarize(mean_mass=mean(mass, na.rm=T)) %>% 
  ggplot(aes(x=reorder(order, mean_mass), y=mean_mass))+ #reorder arranges the data by the x variable, here it is by mass
  geom_col()+
  coord_flip()+
  scale_y_log10()
```
...
elephants %>% 
  ...
  labs(title="Elephant age vs. Height",
       x="Age",
       y="Height")+
  theme(plot.title=element_text(size=rel(1.25), hjust=0.5)) #size_rel makes the title of the plot bigger or smaller
```
...
```{r}
life_history %>% 
  ggplot(aes(x=gestation, y=mass, size=mass))+
  geom_point(na.rm=T)

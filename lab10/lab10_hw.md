...
```
Yes, the data are tidy(wide format).
...
## # … with 30 more rows
```
Most frequently sampled is Merriami and least frequently sampled are viridis, uniparens, tigris, tereticaudus, scutalatus, clarki.


```r
count(deserts, sort=T)
```
...
deserts %>% 
  ...
  labs(title = "No. of merriami species by sex",
       x= "sex",
       y = "No. of Species")
```
